    ...
    function fr:LoadParty(mode)
        ...
        if mode == PARTY_MODE_BASE then
            ...
            UpPnl.Paint = function(self, x, y)
                ...
                if self:IsHovered() then
                    ...
                    DrawShadowText("|", font_get(13), nextpos - 1, y * 0.5 - 1, Color(255, 0, 0), TEXT_ALIGN_CENTER,
                        TEXT_ALIGN_CENTER)
                    nextpos = nextpos - x3 * 2
                    ...
                    if cx >= nextpos and cx < (nextpos + my) then
                        self:SetCursor("hand")
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_esp, c_base, Color(0, 0, 0),
                            0)
                        self.clksel = "esp"
                        nextpos = nextpos - x3
                    else
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_esp, Color(255, 255, 255),
                            Color(0, 0, 0), 0)
                        nextpos = nextpos - x3
                    end
                    ...
                    if cx >= nextpos and cx < (nextpos + my) then
                        self:SetCursor("hand")
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_headset, c_base,
                            Color(0, 0, 0), 0)
                        self.clksel = "head"
                    else
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_headset,
                    ...
                    if cx >= nextpos and cx < (nextpos + my) then
                        self:SetCursor("hand")
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_microphone, c_base,
                            Color(0, 0, 0), 0)
                        self.clksel = "micro"
                    else
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_microphone,
                            Color(255, 255, 255), Color(0, 0, 0), 0)
                    end
                ...
                DrawShadowText(LocalPlayer():Nick(), font_get(9), nextpos, self:GetTall() * 0.5, Color(255, 255, 255),
            ...
            for k, ply in pairs(party_info.members) do
                ...
                pMain.Paint = function(self, x, y)
                    local ply_muted = (party_info.CMute[k] and party_info.CMute[k][mesteamid64]) and true or false
                    local me_muted = (party_info.CMute[mesteamid64] and party_info.CMute[mesteamid64][k]) and true or
                    false
                    ...
                    if self:IsHovered() and not isme then
                        ...
                        if meisleader then
                            ...
                            if cx >= nextpos and cx < (nextpos + my) then
                                self:SetCursor("hand")
                                DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_cross, c_base,
                                    Color(0, 0, 0), 0)
                                self.clksel = "kick"
                            else
                                DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_cross,
                                    Color(255, 255, 255), Color(0, 0, 0), 0)
                            end
                        end
                    ...
                    if ply_muted then
                        nextpos = nextpos - x2 - x3 * 3
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_microphone_off,
                            Color(255, 255, 0), Color(0, 0, 0), 0)
                    ...
                    if me_muted then
                        nextpos = nextpos - x2 - x3 * 3
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_headset_off,
                            Color(255, 255, 0), Color(0, 0, 0), 0)
                    ...
                    DrawShadowText(nick, font_get(7), nextpos, self:GetTall() * 0.5, Color(255, 255, 255),
                        TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
                end
        ...
        elseif mode == PARTY_MODE_NIL then
            ...
            cPnl.Paint = function(self, x, y)
                ...
                DrawShadowText("Create Party", font_get(10), x - self.avzsize - x3 * 3, self.avzsize * 0.5 - 1,
                    Color(255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
            end
            cPnl.OnMousePressed = function(self, key)
                if key != MOUSE_LEFT then return end
                net.Start("jrParty.Create")
                net.SendToServer()
            end
            ...
            imain.Paint = function(self, x, y)
                ...
                DrawShadowText("Invites", font_get(10), x - self.avzsize - x3 * 3, self.avzsize * 0.5 - 1,
                    Color(255, 255, 255, self.alp), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, Color(0, 0, 0, self.alp))

                DrawShadowText(inv_count, font_get(10), x - self.avzsize * 0.5 - x3, self.avzsize * 0.5 - y3,
                    Color(255, 255, 255, self.alp), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, Color(0, 0, 0, self.alp))
            end
            ...
            function fr:InviteAdd(id, ply)
                ...
                iPnl.Paint = function(self, x, y)
                    ...
                    if cx >= nextpos and cx < (nextpos + my) and (cy > 0) and (cy < my) then
                        self:SetCursor("hand")
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5 + 1, y * 0.5, my - 2, my - 2, mat_circle,
                            Color(0, 200, 0), Color(0, 0, 0, 100), 0)
                        self.clksel = "accept"
                    else
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5 + 1, y * 0.5, my - 2, my - 2, mat_circle,
                            Color(255, 255, 255), Color(0, 0, 0, 100), 0)
                    end
                    ...
                    if cx >= nextpos and cx < (nextpos + my) and (cy > 0) and (cy < my) then
                        self:SetCursor("hand")
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_cross, Color(200, 0, 0),
                            Color(0, 0, 0, 100), 0)
                        self.clksel = "cancel"
                    else
                        DrawShadowTexturedRectRotated(nextpos + my * 0.5, y * 0.5, my, my, mat_cross, Color(255, 255, 255),
                            Color(0, 0, 0, 100), 0)
                    end

                    nextpos = nextpos - x3 * 3
                    DrawShadowText(self.nick, font_get(7), nextpos, self:GetTall() * 0.5, Color(255, 255, 255),
                        TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
                end
...
function jrParty.Mark(ply, pos)
    ...
    if not jrParty.MarkEnabled then
        notification.AddLegacy("[Party] Mark Disabled", 0, 5)
        return
    end
    -- EmitSound("party/ping.wav", pos, 1, CHAN_AUTO, 1, 75, 0, 100 )
    chat.PlaySound()
    hook.Add("HUDPaint", "jrParty.Mark", function()
        if not party_info or not party_info.members then
            hook.Remove("HUDPaint", "jrParty.Mark")
            return
        end
        local me_pos = LocalPlayer():GetPos()
...
function jrParty.ESP(status)
    if not status then
        hook.Remove("HUDPaint", "jrParty.ESP")
        jrParty.esp_enabled = false
        return
    end
    local mx, my = ScrW() * 0.02, ScrH() * 0.03
    hook.Add("HUDPaint", "jrParty.ESP", function(self)
        if not party_info.members then
            hook.Remove("HUDPaint", "jrParty.ESP")
            jrParty.esp_enabled = false
            return
        end
        ...
        for k, ply in pairs(party_info.members) do
            ...
            if d < 1500 then
                ...
                DrawShadowText(ply:Nick(), font_get(8), sx, sy, Color(255, 255, 255), TEXT_ALIGN_CENTER,
                    TEXT_ALIGN_CENTER)
            else
