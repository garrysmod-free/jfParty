...
resource.AddWorkshop("2133916559")
resource.AddWorkshop("2049617805") -- map for screenhots
...
net.Receive("jrParty.Create", function(len,ply)
    ...
    if jrParty.Parties[id] then Party_Notify(ply,"You have already created a party!") return end
    if jrParty.Players[id] then Party_Notify(ply,"You are already in the party!") return end
    jrParty.Create(ply)
end)

function jrParty.Leave(ply)
    local pid = ply:SteamID64()
    if not jrParty.Players[pid] then Party_Notify(ply,"You are not a party member") return end
...
function jrParty.Kick(leader,ply)
    ...
    if not jrParty.Players[pid] then Party_Notify(leader, "This player are not a party member") return end
    if not jrParty.Players[lid] then Party_Notify(leader, "You are not a party member") return end
    if not jrParty.Parties[lid] then Party_Notify(leader, "Only a party leader can kick") return end
    if pid == lid then Party_Notify(leader, "You can't kick yourself") return end
...
function jrParty.Invite(leader, ply)
    local lid = leader:SteamID64()
    local pid = ply:SteamID64()
    if not jrParty.Parties[lid] then Party_Notify(leader, "You are not a party leader") return end
...
function jrParty.Accept(ply,lid)
    local pid = ply:SteamID64()
    if jrParty.Players[pid] then Party_Notify(ply, "You are already a party member") return end
    if not jrParty.Parties[lid] then Party_Notify(ply, "Party does not exist") return end
...
net.Receive("jrParty.Accept",function(len,ply)
    ...
    if partyid == "" then Party_Notify(ply,"Party does not exists") return end
...
function jrParty.VoiceChange(ply,micro,head)
    local pid = ply:SteamID64()
    local id = jrParty.Players[pid]
    if not id then Party_Notify(ply,"You are not a party member") return end
    if not jrParty.Parties[id] then Party_Notify(ply,"You are not a party member") return end
    local leader = jrParty.Parties[id].leader
...
function jrParty.Mute(ply,who,mute)
    ...
    if not id then Party_Notify(ply,"You are not a party member") return end
    if not jrParty.Parties[id] then Party_Notify(ply,"You are not a party member") return end
    if jrParty.Players[who_id] != id then Party_Notify(ply,"This player is not in your party") return end
    local leader = jrParty.Parties[id].leader
...
function jrParty.Mark(ply, eye)
    local pid = ply:SteamID64()
    local lid = jrParty.Players[pid]
    if not lid then Party_Notify(ply, "You are not a party member") return end
...
net.Receive("jrParty.Mark",function(len,ply)
    ...
    if not eye.HitPos or eye.HitPos == Vector(0, 0, 0) then Party_Notify(ply, "Position not found") return end
